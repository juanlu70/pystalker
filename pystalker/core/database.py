...
class Database:
    ...
    def _run_migrations(self):
        ...
        cursor.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('db_version', '2')")
        self.conn.commit()
    ...
    def _deduplicate_indicators(self):
        ...
        for symbol in symbols:
            settings_table = f'"{symbol}_settings"'
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (f'{symbol}_settings',))
    ...
    def _ensure_symbol_tables(self, symbol: str):
        if symbol in self._ensured_tables:
            return
        ...
        self.conn.commit()
        self._ensured_tables.add(symbol)
    
    def _bars_table(self, symbol: str, interval: str = '1d') -> str:
        if interval == '1d':
            return f'"{symbol}_bars"'
        return f'"{symbol}_{interval}_bars"'
    
    def _ensure_bars_table(self, symbol: str, bars_table: str):
        cache_key = bars_table.strip('"')
        if cache_key in self._ensured_bars_tables:
            return
        ...
        try:
            ...
            pass
        self.conn.commit()
        self._ensured_bars_tables.add(cache_key)
    ...
    def save_bars(self, bar_data: BarData, interval: str = '1d'):
        ...
        bars_table = self._bars_table(symbol, interval)
        self._ensure_bars_table(symbol, bars_table)
        
        df = bar_data.to_dataframe()
        has_series2 = 'Series2' in df.columns
        series2_values = df['Series2'].values if has_series2 else None
        
        if has_series2 and series2_values is not None:
            rows = []
            for i, bar in enumerate(bar_data.bars):
                s2 = series2_values[i] if i < len(series2_values) else None
                s2_val = float(s2) if s2 is not None and not (isinstance(s2, float) and pd.isna(s2)) else None
                rows.append((
    ...
    def delete_symbol(self, symbol: str):
        ...
        for row in cursor.fetchall():
            ...
            self._ensured_bars_tables.discard(row[0])
        ...
        self._ensured_tables.discard(symbol)
        self.conn.commit()
    ...
    def rename_symbol(self, old_symbol: str, new_symbol: str):
        ...
        for row in cursor.fetchall():
            old_name = row[0]
            suffix = old_name[len(f'{old_symbol}_'):]
            new_name = f'{new_symbol}_{suffix}'
            cursor.execute(f'ALTER TABLE "{old_name}" RENAME TO "{new_name}"')
            self._ensured_bars_tables.discard(old_name)
        ...
        self._ensured_tables.discard(old_symbol)
        self._ensured_tables.discard(new_symbol)
        self.conn.commit()
    ...
    def load_session(self) -> tuple:
        ...
        cursor.execute('SELECT key, value FROM session WHERE key IN (?, ?)', ('open_tabs', 'current_tab'))
        results = {row[0]: row[1] for row in cursor.fetchall()}
        
        open_tabs = results.get('open_tabs', '').split(',') if results.get('open_tabs') else []
        current_tab = results.get('current_tab')
    ...
    def load_chart_colors(self, symbol: str) -> dict:
        ...
        cursor.execute(
            f'SELECT key, value FROM {settings_table} WHERE key IN (?, ?)',
            ('bull_color', 'bear_color')
        )
        result = dict(cursor.fetchall())
        
        colors = {}
        if 'bull_color' in result:
            colors['bull_color'] = result['bull_color']
        if 'bear_color' in result:
            colors['bear_color'] = result['bear_color']
